...
class SelfAttentionLayer(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, dropout: float = 0.0):
        ...
        '''
        super().__init__()
        # Multi-head self-attention over the same input tensor (Q=K=V=x).
        self.mha = nn.MultiheadAttention(d_model, num_heads, dropout=dropout, batch_first=True)
        # Pre-LN normalization.
        self.norm = nn.LayerNorm(d_model)
        # Dropout on the sublayer output before residual add.
    ...
    def forward(self, x: torch.Tensor, key_padding_mask: Optional[torch.Tensor] = None, attn_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        '''
        # Save residual, normalize input, then apply self-attention.
        residual = x
        x_norm = self.norm(x)
...
class CrossAttentionLayer(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, dropout: float = 0.0):
        ...
        # Pre-LN normalization.
        self.norm = nn.LayerNorm(d_model)
        # Dropout on attention output before residual add.
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, key_padding_mask: Optional[torch.Tensor] = None, attn_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            ...
            mha_attn_weights (torch.Tensor): Attention weights, shape (batch_size, seq_len, seq_len)   
        '''
        # Normalize decoder input, then attend over encoder states.
        residual = x
        x_norm = self.norm(x)
        attn_out, mha_attn_weights = self.mha(
            x_norm,
...
class FeedForwardLayer(nn.Module):
    ...
    def __init__(self, d_model: int, d_ff: int, dropout: float = 0.0):
        ...
        # Pre-LN normalization.
        self.norm = nn.LayerNorm(d_model)
        # Dropout on FFN output before residual add.
        self.dropout = nn.Dropout(dropout)
