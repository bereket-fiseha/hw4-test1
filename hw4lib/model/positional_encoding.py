...
class PositionalEncoding(nn.Module):
    ...
    def create_pe_table(self, d_model, max_len):
        ...
        Args:
            ...
            max_len (int): The maximum length of the input sequence.
        """
        # Create position indices and frequency scales for sinusoidal encoding.
        position = torch.arange(max_len, dtype=torch.float32).unsqueeze(1)  # (max_len, 1)
        div_term = torch.exp(
            torch.arange(0, d_model, 2, dtype=torch.float32) * (-math.log(10000.0) / d_model)
        )  # (d_model/2,)

        # Fill even dims with sin and odd dims with cos.
        pe = torch.zeros(max_len, d_model, dtype=torch.float32)
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)

        # Add batch dimension so it can broadcast with (B, T, d_model).
