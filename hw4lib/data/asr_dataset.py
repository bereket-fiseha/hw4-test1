...
class ASRDataset(Dataset):
    def __init__(
        ...
        subset_size = max(1, subset_size) if len(self.fbank_files) > 0 else 0
        ...
        for i in tqdm(range(self.length)):
            ...
            # If loaded as (time, num_feats), transpose to (num_feats, time).
            if feat.ndim == 2 and feat.shape[0] != self.config['num_feats'] and feat.shape[1] == self.config['num_feats']:
                feat = feat.T
            ...
            if self.partition != "test-clean":
                ...
                transcript_raw = np.load(transcript_path, allow_pickle=True)
                if isinstance(transcript_raw, np.ndarray):
                    if transcript_raw.ndim == 0:
                        transcript = str(transcript_raw.item())
                    else:
                        transcript = ''.join(transcript_raw.tolist())
                else:
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.partition != "test-clean":
            ...
            transcript_lengths = torch.LongTensor([seq.size(0) for seq in batch_shifted]) # B  
