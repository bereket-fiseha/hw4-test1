...
class ASRTrainer(BaseTrainer):
    ...
    def _validate_epoch(self, dataloader):
        ...
        """
        # Run greedy recognition on validation data and compare hypotheses to references.
        recognition_config = {
            'num_batches': self.config['training'].get('val_num_batches', None),
            'beam_width': 1,
            'temperature': 1.0,
            'repeat_penalty': 1.0,
            'lm_weight': 0.0,
        ...
        refs = [r['target'] for r in val_results if 'target' in r]
        hyps = [r['generated'] for r in val_results if 'target' in r]

        if len(refs) == 0:
            metrics = {'word_dist': 0.0, 'wer': 0.0, 'cer': 0.0}
        else:
